...
class Zomato:
    ...
    def restaurant_search(self,offset, query="", latitude="", longitude="", cuisines="" ,limit=5):
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines)+"&start="+str(offset)+"&sort=rating&order=desc", headers=headers).content).decode("utf-8")
        return r#a = ast.literal_eval(r)
